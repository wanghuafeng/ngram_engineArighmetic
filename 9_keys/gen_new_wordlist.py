...
def cut_varify_sample_into_sentence():
    '''利用字表（5322）去切割原始验证语料，生成句子——字表+原始语料====>>验证句子语料'''
    linguistic_sample_filename = os.path.join(PATH, '0709modify', 'bak', 'jane.txt')
    cuted_sentence_filename = os.path.join(PATH, '0709modify', 'bak', 'cuted_jane_sentence.txt')
    word_list_filename = os.path.join(PATH, '0709modify', 'single_word_from_95K_5322.txt')
    format_str = ''.join([item.strip() for item in codecs.open(word_list_filename, encoding='utf-8').readlines()])
    wordlist_pattern =re.compile(ur"([%s]+)"%format_str, re.U)
    with codecs.open(linguistic_sample_filename, encoding='utf-8') as f, \
        codecs.open(cuted_sentence_filename, mode='wb', encoding='utf-8') as wf:
        while 1:
            ...
            for param in splited_line_list:
                ...
                if wordlist_pattern.match(param):
                    wf.write(param+'\n')
# cut_varify_sample_into_sentence()
class ClearCutedSentence:
    def __init__(self):
        self.cuted_sentence_filename = os.path.join(PATH, '0709modify', 'bak', 'cuted_jane_sentence.txt')
        self.remove_repeat_word()
    def remove_repeat_word(self):
        with codecs.open(self.cuted_sentence_filename, encoding='utf-8') as f:
            line_set = set([item for item in f.readlines() if len(item)>2])
        codecs.open(self.cuted_sentence_filename, mode='wb', encoding='utf-8').writelines(line_set)
# ccs = ClearCutedSentence()

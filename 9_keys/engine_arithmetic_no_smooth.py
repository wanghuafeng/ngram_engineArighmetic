...
class WordWeight:
    def __init__(self):
        self.src_data_file_path = r'E:\SVN\language_model\9_keys\0709modify\cuted_linguistic_stample'
        self.combine_bigram_freq_filename = 'combine_bigram_freq_delete_2_inorder.txt'
    def total_bigram_freq_count(self):
        '''计算二元模型频度之和'''
        total_freq = 0
        filename = os.path.join(self.src_data_file_path, self.combine_bigram_freq_filename)
        with codecs.open(filename, encoding='utf-8') as f:
            for line in f.readlines():
                splited_line = line.split('\t')
                freq_int = int(splited_line[-1])
                total_freq += freq_int
        print total_freq#869788684
# ww = WordWeight()
# ww.total_bigram_freq_count()
...
class EngineArithmetic:
    ...
    def _load_word_weigh(self):
        ...
        with codecs.open(word_weight_filename, encoding='utf-8') as f:
            words_weight_list = [(item.split('\t')[0], int(item.split('\t')[1])/869788684.0) for item in f.readlines()]
                ...
                for mapping_word in key_mapping_word_list:
                    for last_matched_route_param in matched_route_list:
                        ...
                        if last_matched_route_param[1]:
                            ...
                            weight = self.word_weight_dic.get(bigram_item, 0)
...
# ea = EngineArithmetic()
# key_input = '64242663443664942643343694264'
...
def caculate_parameter():
    ...
    checkout_sample_filename_backward = os.path.join(PATH, '0709modify', 'cut_path_lenght_limit_50_no_smooth.txt')

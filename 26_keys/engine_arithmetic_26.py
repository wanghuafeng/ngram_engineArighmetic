...
class EngineArithmetic:
    ...
    def _real_prefix_rolenum(self):
        '''加载所有真前缀组合'''
        real_prefix_filename = os.path.join(PATH, 'data', 'src', 'prefix_if_mapping_role_num.txt')
    ...
    def _load_mapping_rolenum_wordlist(self):
        ...
        mapping_filename = os.path.join(PATH, 'data', 'src', 'combine_5233_and_top60000_pinyin_role_inorder_mapping.txt')
    ...
    def _load_word_weigh(self):
        ...
        word_weight_filename = os.path.join(PATH, 'data', 'src', 'combine_bigram_freq_weight.txt')
...
# ea = EngineArithmetic()
# key_input = 'wojintianzaoshangchilezaofan'
...
def caculate_parameter():
    ea = EngineArithmetic()
    start_time = time.time()
    varify_sample_filename = os.path.join(PATH, 'data', 'cuted_varify_sample_pinyin_role_no_repeat.txt')
    checkout_sample_filename_backward = os.path.join(PATH, 'data', 'cut_path_lenght_26_keys_limit_100.txt')
...
def multiprocess_caculate_parameter():
    ea = EngineArithmetic()
    start_time = time.time()
    varify_sample_filename = os.path.join(PATH, '0709modify', 'cuted_varify_sample_pinyin_role_no_repeat.txt')
    checkout_sample_filename_backward = os.path.join(PATH, '0709modify', 'cut_path_lenght_limit_500.txt')
    assert os.path.isfile(varify_sample_filename)
    with codecs.open(varify_sample_filename, encoding='utf-8') as f,\
    codecs.open(checkout_sample_filename_backward, mode='a', encoding='utf-8') as wf:
        count = 0
    ...
    end_time = time.time()
    print end_time-start_time
# multiprocess_caculate_parameter()

...
class GenPercentage:
    ...
    def gen_first_word_total_freq_dic(self):
        ...
        filename = os.path.join(PATH, '0709modify', 'cuted_linguistic_stample', 'combine_bigram_freq_inorder.txt')
        ...
        with codecs.open(filename, encoding='utf-8') as cf:
            ...
            while 1:
                ...
                line = cf.readline()
                if not line:
                    # print c, first_word_equal_in_bigram_count, freq_int
                    freq_int += 61634
                ...
                if next_word != first_word:
                    if not first_word:
                        ...
                        if first_word == 'BOS':
                            ...
                            freq_int += 61634
                        # print first_word_equal_in_bigram_count, freq_int
                        total_frist_word_freq_dic[first_word] = freq_int
                        # print total_frist_word_freq_dic
                        # time.sleep(1)
                        first_word = next_word
                        freq_int = int(splited_line[1])
                        first_word_equal_in_bigram_count = 1
                    ...
                    # print line.strip()
                    # time.sleep(1)
                # print c, first_word_equal_in_bigram_count
    def gen_percentage_of_first_word(self):
        ...
        src_filename = os.path.join(PATH, '0709modify', 'cuted_linguistic_stample', 'combine_bigram_freq_inorder.txt')
    ...
    def add_omited_bigram_param(self):
        ...
        codecs.open(test_filename, mode='wb', encoding='utf-8').writelines(bigram_weight_list)

# gp = GenPercentage()
# gp.add_omited_bigram_param()
def get_high_freq_word():
    '''获取词频高于1的二元组数据'''
    combine_bigram_filename = os.path.join(PATH, 'cuted_linguistic_stample', 'combine_bigram_freq.txt')
    with codecs.open(combine_bigram_filename, encoding='utf-8') as f:
        high_freq_list = [item for item in f if item.split('\t')[1].strip() != '1']
        combine_bigram_remove_freq_1_filename = os.path.join(PATH, 'cuted_linguistic_stample', 'combine_bigram_remove_freq_1.txt')
        codecs.open(combine_bigram_remove_freq_1_filename, mode='a', encoding='utf-8').writelines(high_freq_list)
# get_high_freq_word()
def check_dic_usage():
    '''查看CPU耗用资源的问题'''
    total_list = []
